Function Update-Appearance {
    ...
    SetBackgroundForDesktop -Picture "$Picture"
    SetBackgroundForLockscreen -Picture "$Picture"
...
Function Update-Windows {
    ...
    RunActivateWindows
    SetHostname -Payload "GAMHOGEN"
    SetSoundVolume -Payload 40
    SetTimezone -Payload "Romance Standard Time"
...
Function Update-Firefox {
    ...
    $Current = GetFileVersion "$Env:ProgramFiles\Mozilla Firefox\firefox.exe"
...
Function Update-Heroic {
    ...
    $Current = GetFileVersion "$Env:LocalAppData\Programs\heroic\Heroic.exe"
    ...
    RunRemoveFromDesktop -Pattern "Heroic*.lnk"

}
Function Update-Hydra {

    $Current = GetFileVersion "$Env:LocalAppData\Programs\Hydra\Hydra.exe"
    ...
    RunRemoveFromDesktop -Pattern "Hydra*.lnk"
...
Function Update-Nvidia {
    ...
    $Current = GetFileVersion "*nvidia*graphics*driver*"
    ...
    If (-Not $Updated) {
        ...
        $Fetched = Join-Path "$([System.IO.Path]::GetTempPath())" "$(Split-Path "$Address" -Leaf)"
        (New-Object Net.WebClient).DownloadFile("$Address", "$Fetched")
        $Extract = RunExpandArchive "$Fetched"
...
Function Update-Steam {
    ...
    RunRemoveFromDesktop "Steam*.lnk"
